...
scale_continuous <- function(data, to_scale) {
  ...
  # TEN WARUNEK SPRAWDZA TYLKO PIERWSZĄ WARTOŚĆ WEKTORZE to_scale, 
  # kod się wysypuje np. dla c('Bedrooms','Bathrooms','AreaNet', NA)
  if (!is.na(to_scale)) {
...
train_test_split <- function(data, target, train_size =0.6){
  ...
  # POWINIEN TU BYĆ UŻYTY PARAMETR train_size
...
# NALEŻY UŻYĆ Metrics::rmse inaczej rzuca błędem
...
data = data[c(3, 4, 5, 14, 36, 37, 38, 39)]
colnames(data)
data     <- preprocessing(data, "BuyPrice", to_scale = c(2, 3, 5, 6, 7, 8))
data     <- train_test_split(data = data, target = "Price",  train_size = 0.7)
Xy_train <- cbind(data$train_x, data$train_y)
Xy_test  <- cbind(data$test_x, data$test_y)
X_train  <- data$train_x
